...
private def runCheck(dir: Path): Unit =
  ...
  if unmatched.isEmpty then println("✓ All steps matched.")
  else
...
private def runSnippet(dir: Path): Unit =
  val unmatched = Scanner.unmatchedSteps(dir)
  if unmatched.isEmpty then println("// All steps are already matched — no skeletons to generate.")
  else
...
private def runList(dir: Path): Unit =
  ...
  if defs.isEmpty then println("No step definitions found.")
  else
    defs.foreach { d =>
      val rel =
        try dir.relativize(Path.of(d.file)).toString
        catch case _: Exception => d.file
